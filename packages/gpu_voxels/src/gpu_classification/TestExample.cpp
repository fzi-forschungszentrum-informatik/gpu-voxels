...
void reconfigureCallback(classificator_reconfigure::ClassificatorConfig &config, uint32_t level)
{
  ...
  heightCheck = config.additionalTransZ;
}

void kinectSimulatorCallback(const PointCloud::ConstPtr& msg)
{
  ...
  {
    ROS_ERROR("kinectSimulatorCallback::pointCloudCB: Could not find a transform from 'cam' to 'world' for 5 seconds!");
    return;
  }
  ...
  {
    if(m_global_kinect_cloud.points[i].z < heightCheck)
    {
      groundCounter++;
    }
  ...
  //LOGGING_INFO(gpu_voxels::classification::TestExample, "Number of counted Ground Points: " << groundCounter << gpu_voxels::classification::endl);


  std::vector<gpu_voxels::Vector3f> ground;
  std::vector<std::vector<gpu_voxels::Vector3f> > objects;
  if(!displayReference)
  {
    try
    {
      LOGGING_INFO(gpu_voxels::classification::TestExample, "Start Segmentation" << gpu_voxels::classification::endl);
      segmentation.segmentPointCloud(point_cloud);
      LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError after Segmentation: " << cudaGetLastError() << gpu_voxels::classification::endl);
    }
    catch(thrust::system_error &e)
    {
      LOGGING_ERROR(gpu_voxels::classification::TestExample, "Thrust Error in Segmentation: " << e.what() << gpu_voxels::classification::endl);
    }
    ...
    LOGGING_INFO(gpu_voxels::classification::TestExample, "Finished Segmentation" << gpu_voxels::classification::endl);
  ...
  {
    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Add1: " << cudaGetLastError() << gpu_voxels::classification::endl);

    LOGGING_INFO(gpu_voxels::classification::TestExample, "Adding " << (int)ground.size() << " Points as ground" << gpu_voxels::classification::endl);
    mpc.addCloud(ground, false, "GroundCloud");

    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Add2: " << cudaGetLastError() << gpu_voxels::classification::endl);

    LOGGING_INFO(gpu_voxels::classification::TestExample, "Adding " << ((int)point_cloud.size() - (int)ground.size()) << " of Points in " << (int)objects.size() << " Segments" << gpu_voxels::classification::endl);
    ...
    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Add3: " << cudaGetLastError() << gpu_voxels::classification::endl);

  }
  else
  {
    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Update1: " << cudaGetLastError() << gpu_voxels::classification::endl);

    LOGGING_INFO(gpu_voxels::classification::TestExample, "Updating " << (int)ground.size() << " Points as ground" << gpu_voxels::classification::endl);
    mpc.updatePointCloud("GroundCloud", ground, false);

    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Update2: " << cudaGetLastError() << gpu_voxels::classification::endl);

    LOGGING_INFO(gpu_voxels::classification::TestExample, "Updating " << ((int)segmentation.getNumberOfSegmentedPoints()) << " of Points in " << (int)objects.size() << " Segments" << gpu_voxels::classification::endl);
    ...
    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Update3: " << cudaGetLastError() << gpu_voxels::classification::endl);

    currentNumberOfPointclouds = mpc.getNumberOfPointclouds();
    LOGGING_INFO(gpu_voxels::classification::TestExample, "Number of Objects: " << (int)objects.size() << " Number of Clouds: " << currentNumberOfPointclouds << gpu_voxels::classification::endl);
    for(uint i = 0; i < currentNumberOfPointclouds - 1; i++)
    {
      ...
      if(i < objects.size())
      {
        ...
        //LOGGING_INFO(gpu_voxels::classification::TestExample, "Object " << i << " Size: " << (int)objects[i].size() << gpu_voxels::classification::endl);
      }
      else
      {
    ...
    LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Update4: " << cudaGetLastError() << gpu_voxels::classification::endl);
  }
  ...
  for(uint i = 1; i < objectMeanings.size(); i++)
  {
    ...
    objectMeanings[i] = static_cast<gpu_voxels::BitVoxelMeaning>(11);
    tempMeaning++;
  }

  //visualize pointcloud
  gpu_voxels::Matrix4f translateMatrix;
  translateMatrix.a14 = 5.0f;
  translateMatrix.a24 = 5.0f;
  translateMatrix.a34 = 0.0f;
  ...
  LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Transform1: " << cudaGetLastError() << gpu_voxels::classification::endl);


  gpu->transformPointCloud(&translateMatrix, &mpc, transformMPC);

  LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError Transform2: " << cudaGetLastError() << gpu_voxels::classification::endl);

  gvl->clearMap("myTestExampleMap");

  LOGGING_TRACE(gpu_voxels::classification::TestExample, "GetLastError before InserMetaPointcloud: " << cudaGetLastError() << gpu_voxels::classification::endl);

  gvl->getMap("myTestExampleMap")->insertMetaPointCloud(*transformMPC, objectMeanings);

  gvl->visualizeMap("myTestExampleMap");
...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "testexample_node", ros::init_options::NoSigintHandler);
  ...
  gvl->addMap(gpu_voxels::MT_BITVECTOR_VOXELLIST, "myTestExampleMap");


  gpu = new gpu_voxels::classification::IbeoNcomFilterGPU("myTestExampleMap");
  ...
  segmentation.setWithClassification(true);
  ...
  ros::NodeHandle n;
  ros::Subscriber kinect_sub = n.subscribe("kinect_simulator", 1, kinectSimulatorCallback);
  ...
  LOGGING_INFO(gpu_voxels::classification::TestExample, "Finished Initialization" << gpu_voxels::classification::endl);
