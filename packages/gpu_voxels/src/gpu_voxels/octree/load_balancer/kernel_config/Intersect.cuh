...
//template<std::size_t num_threads, std::size_t branching_factor, typename InnerNode1, typename LeafNode1,
//    typename InnerNode2, typename LeafNode2, bool set_collision_flag, typename Collider>
//__global__ void kernelIntersectNTrees(WorkItem* work_stacks, uint32_t* work_stacks_item_count,
//                                      const uint32_t stack_size_per_task, uint32_t* tasks_idle_count,
//                                      const uint32_t idle_count_threshold, const uint32_t min_level,
//                                      std::size_t* num_collisions, Collider collider)
//{
//  typedef WorkItemIntersect<InnerNode1, InnerNode2> WorkItem;
//
...
//  const uint32_t work_index = thread_id / branching_factor;
//  const uint32_t work_lane = thread_id % branching_factor;
//
...
//  // ###########################################################
//
//  volatile __shared__ uint32_t shared_block_votes[num_threads / WARP_SIZE];
//
...
//              num_work_items * sizeof(WorkItem), thread_id, num_threads);
//    __syncthreads();
//
...
//    if (is_active)
//    {
//      a_active = shared_work_item_cache[work_index].a_active;
//      a_node = shared_work_item_cache[work_index].a;
//      a_node += work_lane * a_active;
//      b_active = shared_work_item_cache[work_index].b_active;
//      b_node = shared_work_item_cache[work_index].b;
//      b_node += work_lane * b_active;
//      end_here = (min_level >= shared_work_item_cache[work_index].level);
//      is_last_level = (a_node->hasStatus(ns_LAST_LEVEL) | b_node->hasStatus(ns_LAST_LEVEL)) & !end_here;
//      insert_work_item = collider.collide(*a_node, *b_node) & !is_last_level
//          & (a_node->hasStatus(ns_PART) | b_node->hasStatus(ns_PART)) & !end_here;
//    }
//
//    // the only active search is ending here
//    if (is_active
//        && (collider.collide(*a_node, *b_node)
//            & ((!a_node->hasStatus(ns_PART) & !b_node->hasStatus(ns_PART)) | end_here)))
//    {
//      my_num_collisions += const_voxel_at_level[shared_work_item_cache[work_index].level];
//      if (set_collision_flag)
//      {
//        if (a_active)
//          a_node->setStatus(a_node->getStatus() | ns_COLLISION);
...
//
//    // handle leaf nodes
//    if (__syncthreads_or(is_last_level) && min_level == 0)
//    {
//      InnerNode1* a_temp;
//      InnerNode2* b_temp;
//      const uint32_t leafs_per_work = branching_factor * branching_factor;
//      for (uint32_t i = thread_id; i < num_work_items * leafs_per_work; i += num_threads)
//      {
//        const uint32_t my_work_item = i / leafs_per_work;
//        const uint32_t my_inner_node = (i % leafs_per_work) / branching_factor;
//        a_temp = shared_work_item_cache[my_work_item].a;
//        const bool a_temp_active = shared_work_item_cache[my_work_item].a_active;
//        a_temp += my_inner_node * a_temp_active;
//        b_temp = shared_work_item_cache[my_work_item].b;
//        const bool b_temp_active = shared_work_item_cache[my_work_item].b_active;
//        b_temp += my_inner_node * b_temp_active;
//        if ((shared_work_item_cache[my_work_item].level == 1) & collider.collide(*a_temp, *b_temp)
//            & (a_temp->hasStatus(ns_PART) | b_temp->hasStatus(ns_PART)))
//        {
//          const uint32_t my_leaf_node = i % branching_factor;
//          bool is_in_conflict = false;
...
//            LeafNode1* a = &((LeafNode1*) a_temp->getChildPtr())[my_leaf_node];
//            LeafNode2* b = &((LeafNode2*) b_temp->getChildPtr())[my_leaf_node];
//            is_in_conflict = collider.collide(*a, *b);
//            if (set_collision_flag)
//            {
//              if (is_in_conflict)
//                a->setStatus(a->getStatus() | ns_COLLISION);
...
//          {
//            LeafNode1* a = &((LeafNode1*) a_temp->getChildPtr())[my_leaf_node];
//            is_in_conflict = collider.collide(*b_temp, *a);
//            if (set_collision_flag)
//            {
//              if (is_in_conflict)
//                a->setStatus(a->getStatus() | ns_COLLISION);
...
//          {
//            LeafNode2* b = &((LeafNode2*) b_temp->getChildPtr())[my_leaf_node];
//            is_in_conflict = collider.collide(*a_temp, *b);
//            if (set_collision_flag)
//            {
//              if (is_in_conflict)
//                b->setStatus(b->getStatus() | ns_COLLISION);
//            }
//          }
//          my_num_collisions += is_in_conflict;
//        }
//      }
//    }
//
//    uint32_t all_votes = thread_prefix<num_threads / WARP_SIZE>(shared_block_votes, thread_id,
//                                                                insert_count_tid0, insert_work_item);
//
//    // add new work items to stack
//    if (is_active & insert_work_item)
//    {
//      const uint32_t warp_local_index = __popc(all_votes << (WARP_SIZE - warp_lane));
//      const uint32_t inter_warp_index = shared_block_votes[warp_id];
//      WorkItem* stackPointer = &shared_my_work_stack[shared_num_stack_work_items + inter_warp_index
//          + warp_local_index];
//      a_active &= a_node->hasStatus(ns_PART);
//      InnerNode1* a_children = a_active ? ((InnerNode1*) a_node->getChildPtr()) : a_node;
//      b_active &= b_node->hasStatus(ns_PART);
//      InnerNode2* b_children = b_active ? ((InnerNode2*) b_node->getChildPtr()) : b_node;
//      *stackPointer = WorkItem(a_children, b_children, shared_work_item_cache[work_index].level - 1, a_active,
//                               b_active);
//    }
//    __syncthreads();
//
...
//
//  // todo is a shared memory reduction more efficient in this case?
//  if (my_num_collisions != 0)
//    atomicAdd((unsigned long long int*) num_collisions, (unsigned long long int) my_num_collisions);
//
//  // ###################################################
