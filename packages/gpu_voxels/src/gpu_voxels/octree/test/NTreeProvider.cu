...
NTreeProvider::NTreeProvider() :
    Provider(), m_ntree(NULL), m_min_level(0), m_shm_superVoxelSize(NULL), m_shm_memHandle(NULL), m_shm_numCubes(
        NULL), m_shm_bufferSwapped(NULL), m_fps_rebuild(0), map_data_offset(0), m_spinner(NULL), m_node_handle(
        NULL), m_subscriber_front(NULL), m_subscriber_back(NULL), m_tf_listener(NULL)
{
...
// disable UNKNOWN and FREE
  ...
  uint32_t used_size = m_ntree->extractCubes(cubes_buffer_1, selection_ptr, m_min_level);
...
//        + gpu_voxels::Vector3f(tmp.getX() - 122, tmp.getY() + 82, tmp.getZ());
    ...
    thrust::device_vector<Voxel> d_free_space_voxel;
    thrust::device_vector<Voxel> d_object_voxel;
    m_sensor.processSensorData(points, d_free_space_voxel, d_object_voxel);
