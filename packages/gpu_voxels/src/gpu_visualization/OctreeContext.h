 ...
 * \date    2014-02-10
 *
 * \brief   Saves all necessary stuff to draw an octree (the cube list of the octree).
 */
//----------------------------------------------------------------------
#ifndef GPU_VOXELS_VISUALIZATION_OCTREECONTEXT_H_INCLUDED
#define GPU_VOXELS_VISUALIZATION_OCTREECONTEXT_H_INCLUDED
...
class OctreeContext: public DataContext
{
public:

  OctreeContext(std::string map_name) 
    : m_d_cubes(NULL), 
      m_number_of_cubes(0)
  {
    m_map_name = map_name;
  ...
  OctreeContext(Cube* cubes, uint32_t num_cubes, std::string map_name)
  {
    m_map_name = map_name;
    m_d_cubes = cubes;
  ...
  {
    m_threads_per_block = dim3(1024);
    m_num_blocks = dim3(m_number_of_cubes / 1024 + 1);
  }
